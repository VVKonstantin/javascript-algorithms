 ...
 *
*/
// Решение "влоб". Подходит для не сильно больших чисел. Для больших чисел лучше вывести формулу подсчета нолей математически
function countZeros(n) {
  let count = 0;
  for (let i = 1; i <= n; i++) {
    count += (i + '').split('').reduce(function (prevVal, item) {
      if (item === '0') return ++prevVal;
      else return prevVal;
    }, 0);
  }
  return count;
